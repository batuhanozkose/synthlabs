...
const ReasoningHighlighter: React.FC<ReasoningHighlighterProps> = ({ text }) => {
  if (!text) return <span className="text-slate-600 italic">Waiting for trace...</span>;
  ...
  return (
    <div className="font-mono text-xs md:text-sm leading-relaxed whitespace-pre-wrap text-slate-300">
      {parts.map((part, i) => {
        switch (part) {
          case '→': return <span key={i} className="text-blue-400 font-bold mx-1">→</span>;
          case '↺': return <span key={i} className="text-purple-400 font-bold mx-1">↺</span>;
          ...
          default:
            if (part.startsWith('<H≈')) {
              return <span key={i} className="text-pink-400 font-bold mr-1">{part}</span>;
            }
